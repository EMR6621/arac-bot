...
MIN_YIL=2010
F="gorulmus.json"
PROXIES=["38.154.203.95:5863:gemoikua:q7mqiildydiz","198.105.121.200:6462:gemoikua:q7mqiildydiz","64.137.96.74:6641:gemoikua:q7mqiildydiz"]
 ...
 try:
  ...
  for x in soup.select("tr.searchResultsItem"):
   try:
    ...
    if b and f:il.append({"kaynak":"Sahibinden","b":b.get_text(strip=True),"l":"https://www.sahibinden.com"+b["href"],"f":fp(f),"km":fp(a[1]) if len(a)>1 else 0,"yil":fp(a[0]) if a else 0,"yil_str":a[0].get_text(strip=True) if a else ""})
   except:pass
 ...
 try:
  ...
  for x in soup.select("tr.listing-list-item"):
   try:
    ...
    if b and f:il.append({"kaynak":"Arabam.com","b":b.get_text(strip=True),"l":"https://www.arabam.com"+b["href"],"f":fp(f),"km":fp(km),"yil":fp(y),"yil_str":y.get_text(strip=True) if y else ""})
   except:pass
...
def main():
 ...
 for i in ilanlar:
  ...
  if MIN_ESIK<=yuz<=MAX_ESIK:
   ...
   time.sleep(1)
 gk(g)
 print(f"  {len(ilanlar)} ilan, {yeni} ilk cumartesi.")
 tg(f"🚗 <b>Bot v4.0 Aktif!</b>\n✅ Proxy ile Sahibinden\n✅ Arabam.com\n✅ Maksimum 250.000 Km\n✅ 2010+ veliler\n✅ Onun 5 dakikada bir tarama")
